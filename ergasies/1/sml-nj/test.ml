...
let rec squares_that_have_access (queue, lis, count, square, x, y) = 
	match lis with 
		|[] -> (queue @ square)
		| (h::xs) ->
...
let rec calc_winning_row_squares (n, m, row, count, acc, square) =
	let l = acc @ square in
		if count <> m
				then 
...
let rec calc_winning_col_squares (n, m, col, count, acc, square) = 
	let l = acc @ square in
		if count <> n
			then 
...
let rec calc_winning_squares (n, m, acc, square_list, count) = 
...
let arr = Bigarray.Array2.create Bigarray.char Bigarray.c_layout 3 3;;
let temp = calc_n_m (split_at_space (parse "test_cases/2/file"));;
let n = int_of_string (get_1_3 temp);;
let m = int_of_string (get_2_3 temp);;
let winning_list =  calc_winning_squares (n, m, [], [], 0);;
calc_input (arr, n, m, (get_3_3 temp));;

let list_pop_front l = match l with | [] -> [] | (x::xs) -> xs;;

let rec print_list_of_tuples l = 
	match l with
	| (a, b)::xs -> Printf.printf "(%d, %d) " a b ; (print_list_of_tuples xs)
	| [] -> Printf.printf "\n";;
...
let rec count_winning_squares (l, count) = 
	match l with 
		|[] -> count
		| (a, b)::xs -> 
			let rec winning_tree_nodes (lid, count) = match lid with 
				...
				| (a, b)::queue  ->
					if (a = 0 && b = 0)
						then winning_tree_nodes (squares_that_have_access (queue, ['N'; 'N'; arr.{a, b + 1}; arr.{a + 1, b}], 0, [], a, b), count + 1)

					else if (a = n - 1 && b = m - 1)
						then winning_tree_nodes (squares_that_have_access (queue, [arr.{a, b - 1}; arr.{a - 1, b}; 'N'; 'N'], 0, [], a, b), count + 1)

					else if (a = 0 && b = m - 1)
						then winning_tree_nodes (squares_that_have_access (queue, [arr.{a, b - 1}; 'N'; 'N'; arr.{a + 1, b}], 0, [], a, b), count + 1)

					else if (a = n - 1 && b = 0)
						then winning_tree_nodes (squares_that_have_access (queue, ['N'; arr.{a - 1, b}; arr.{a, b + 1}; 'N'], 0, [], a, b), count + 1)

					else if a = 0
						then winning_tree_nodes (squares_that_have_access (queue, [arr.{a, b - 1}; 'N'; arr.{a, b + 1}; arr.{a + 1, b}], 0, [], a, b), count + 1)

					else if a = n - 1
						then winning_tree_nodes (squares_that_have_access (queue, [arr.{a, b - 1}; arr.{a - 1, b}; arr.{a, b + 1}; 'N'], 0, [], a, b), count + 1)

					else if b = 0
						then winning_tree_nodes (squares_that_have_access (queue, ['N'; arr.{a - 1, b}; arr.{a, b + 1}; arr.{a + 1, b}], 0, [], a, b), count + 1)

					else if b = m - 1
						then winning_tree_nodes (squares_that_have_access (queue, [arr.{a, b - 1}; arr.{a - 1, b}; 'N'; arr.{a + 1, b}], 0, [], a, b), count + 1)

					else
						winning_tree_nodes (squares_that_have_access (queue, [arr.{a, b - 1}; arr.{a - 1, b}; arr.{a, b + 1}; arr.{a + 1, b}], 0, [], a, b), count + 1)
		in	
