...
fun get_list (x, arr) = Array.sub(arr, x)
    ...
    let
      ...
      fun make_array (0, arr) = arr
        | make_array (M, arr) =
          ...
          in
          (
            Array.update(arr, a, b::Array.sub(arr, a));
            Array.update(arr, b, a::Array.sub(arr, b));
            make_array(M-1, arr)
          )
          end

      fun readUseless(0) = Array.array(1, [])
        | readUseless(M) =
          let
            val a = readInt inStream
            val b = readInt inStream
      ...
      fun readtotal (0, list) = rev list
        ...
        let
          ...
          val (a,b,c) = if(N = M) then (N, M, make_array(M, Array.array(N+1, [])))
