...
create_graph([First, Second | Rest], Graph):-
    ...
    nb_setarg(First, Graph, [Second | ListFirst]),
    nb_setarg(Second, Graph, [First | ListSecond]),
...
solve_case(_, _, N, M, _, ["'NO CORONA'"]):- write("DIFFERENT: "), write(N), write(" "), writeln(M).
...
read_input(File, Times, Testcases) :-
    ...
    take_head(T, Times),
    read_sizes(Stream, Times, [], Testcases).

take_head([H], Result):-
    Result = H.

read_sizes(_, 0, Result, Result).
read_sizes(Stream, Times, List, Result):-
    ...
    Newtimes is Times-1,
    FinalList = [N, M | [Testcase]],
    append(List, FinalList, NewList),
    read_sizes(Stream, Newtimes, NewList, Result).
...
coronograph(File, Answer):-
    ...
    solve_exercise(Graph, Visited, Tests, Testcases, [], Answer),
    !.
