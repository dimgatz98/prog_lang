%Implementation of queue with difference lists, seen with hashing and association lists. Walls, my laptop and a cat were harmed during the writing of this code :)
...
add_pos(state(First, Second), Encountered, Moves, Curr_Queue, Curr_Visited, 'p', Next_Queue, Next_Visited, EarlyStop):-
    add_if_safe(First, Second, Encountered, NewFirst, NewSecond, NewEncountered, Checker),
    ...
    (
      ...
      same("", NewFirst) ->
        put_assoc(Key, Curr_Visited, State, Next_Visited),
        Next_Queue = [State, NewEncountered, ['p' | Moves]]| X]-X.
      ;
        put_assoc(Key, Curr_Visited, State, Next_Visited),
