...
fun solve filename = 
	let
		...
		fun calc_input (M::N::xs) = 
			let
				...
				val n = (valOf (Int.fromString N) );
				val temp = Array.array (m, 0);
				fun calc_prefix ([], counter) = (n, m, temp)
				|	calc_prefix (h::xxs, counter) = 
						if counter = 0
							then (Array.update (temp, counter, (valOf (Int.fromString h)) + n ); calc_prefix (xxs, counter + 1))
						else	
							(Array.update (temp, counter, Array.sub(temp, counter - 1) + (valOf (Int.fromString h)) + n ); calc_prefix (xxs, counter + 1))
			in
				calc_prefix (xs, 0)
			end
	...
	in
		find_max_subseq (min_arr, max_arr, m, 0, 0, 0)
	end